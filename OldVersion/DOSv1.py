# -*- coding: utf-8 -*-
"""
Created on Fri Mar 18 14:22:45 2022

@author: jungu

calculate the imaginary part of the self-energies using analytic expressions
"""
...
def SigmaW(D, t, w, e):
    ...
    ret += B1*( quad(funcw, -np.inf, 0, args = (D,t,w,e,-1, True),
                    limit=LIMIT)[0]
               + quad(funcw, 0, np.inf, args = (D,t,w,e,1, True),
                      limit=LIMIT)[0] )
    ret *= w
...
def SigmaD(D, t, w, e): # should have D>0
    ret = B0*np.pi/np.sqrt(-w*w+D*D)/np.cosh(e/2/t)**2
    ret += B1*(  quad(funcd, -np.inf, 0, args = (D,t,w,e,-1, True),
                      limit=LIMIT)[0]
               + quad(funcd, 0,  np.inf, args = (D,t,w,e, 1, True),
                      limit=LIMIT)[0] )
    ret += 1j*B1*np.pi/2*(
...
for i in tqdm(range(N)):
    x[1][i] = dos(x[0][i]+1j*delta, D, t, e)
